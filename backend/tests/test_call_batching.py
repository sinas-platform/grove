...
async def test_answer_evidence_nests_rows_under_their_claims():
    ...
    session = _FakeSession([[c1, c2], [e1, e2, e3]])

    out = await get_answer_evidence(answer_id=answer_id, session=session)
